...
DiscourseRecruitTracker::Engine.routes.draw do
  ...
  get "/audit" => "overview#audit", constraints: { format: :html }, defaults: { format: :html }
  get "/audit.json" => "overview#audit_log", defaults: { format: :json }
  put "/users/:id/status" => "users#update_status", defaults: { format: :json }
  post "/users/:id/notes" => "notes#create", defaults: { format: :json }
  post "/manual" => "manual#create", defaults: { format: :json }
  delete "/manual/:id" => "manual#destroy", defaults: { format: :json }
end
